...
def run_prediction_pipeline(image_file) -> Dict[str, Any]:
    ...
    for det in detections:
        ...
        try:
            ...
            CLASSIFICATION_PREDICTIONS_TOTAL.labels(type="variety").inc()
            CLASSIFICATION_CONFIDENCE.labels(type="variety").set(var_res["confidence"])
        except Exception as var_exc:
            CLASSIFICATION_ERRORS.labels(type="variety").inc()
            raise var_exc

        try:
            mat_res = predict_maturity(crop_pil)
            CLASSIFICATION_PREDICTIONS_TOTAL.labels(type="maturity").inc()
            CLASSIFICATION_CONFIDENCE.labels(type="maturity").set(mat_res["confidence"])
        except Exception as mat_exc:
            CLASSIFICATION_ERRORS.labels(type="maturity").inc()
            raise mat_exc
        # ──────────────────────────────────────────────────────────────────
